...
class _EmergencyContactsPageState extends State<EmergencyContactsPage> {
  ...
  Future<void> _fetchEmergencyContacts(String countryCode) async {
    // ⚠️ 비상연락처 API 키 설정 (중요!)
    // 아래의 API 키를 본인의 공공데이터포털 API 키로 변경해야 합니다.
    //
    // 발급 방법:
    // 1. 공공데이터포털 (https://www.data.go.kr/) 접속
    // 2. "해외여행자 안전정보" API 검색
    // 3. API 키 발급 신청
    // 4. 아래의 apiKey 값을 본인의 API 키로 교체
    final String apiKey =
        "YOUR_PUBLIC_DATA_API_KEY_HERE"; // ⚠️ 본인의 공공데이터포털 API 키로 변경하세요!
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 10),
                              if (_selectedCountryData['flag_download_url'] !=
                                  null)
                                Image.network(
                                    _selectedCountryData['flag_download_url'],
                                    height: 60,
                                    width: 100),
                              SizedBox(height: 10),
                              if (_selectedCountryData['contact_remark'] !=
                                  null)
                                Text(
